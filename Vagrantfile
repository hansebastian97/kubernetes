...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "ansible" do |ansible|
    ...
    if BUILD_MODE == "BRIDGE"
      adapter = ""
      ansible.vm.network :public_network, bridge: get_bridge_adapter()
    else
      ansible.vm.network :private_network, ip: IP_NW + ".#{CONTROLNODE_IP_START}"
    end
    ansible.vm.synced_folder ".\\vagrant_folder", "/vagrant"
    ...
    ansible.vm.provision "shell", privileged: false, path: ".\\vagrant_folder\\ansible.sh"
  end 
  ...
  config.vm.define "controlplane" do |node|
    ...
    node.vm.synced_folder ".\\vagrant_folder", "/vagrant"
    if BUILD_MODE == "BRIDGE"
      adapter = ""
